...
def append_to_sheet_sync(trade_data, date_analysed):
    ...
    Columns: A=NAME, B=Gain, C=Buying Price, D=Target, E=Date, F=Reason, G=Current Price, H=Price Source
    ...
    try:
        ...
        # --- Buying Price Logic ---
        # If AI extracted a buying price, use it (Recommended). Otherwise use GOOGLEFINANCE (Auto).
        if buying_price_raw and buying_price_raw != "":
            buying_price = buying_price_raw
            price_source = "Recommended"
        else:
            # Auto-populate from Google Finance current price
            buying_price = f'=GOOGLEFINANCE(A{next_row}, "price")'
        ...
        # Columns: A=Name, B=Gain, C=Buy, D=Target, E=Date, F=Reason, G=Current Price, H=Price Source
